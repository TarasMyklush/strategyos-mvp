(function () {
  ...
  function renderClaim(record) {
    ...
    var details=node('details','');details.append(node('summary','Lineage and assessments'));details.append(node('pre',JSON.stringify({formula:record.formula,assumptions:record.assumptions,assessments:record.assessments,traceability:record.traceability},null,2)));article.append(details);
    return article;
  }
...
try{var response=await fetch(path+'?'+params.toString(),{headers:headers,cache:'no-store'});if(!response.ok)throw new Error(response.status===401?'Please sign in again.':response.status===403?'Your source permissions do not allow this view.':'Evidence is unavailable. No cached values are being shown.');var payload=await response.json();if(version!==requestVersion)return;var records=payload.records || [];records.forEach(function(record){results.append(renderClaim(record));});status.textContent=records.length?'Current authorized claims: '+records.length+' · checked '+payload.analysis_as_of:'No eligible claims match this selection. No other claim type was substituted.';}catch(error){if(version!==requestVersion)return;results.replaceChildren();status.textContent=error instanceof TypeError?'Evidence is unavailable. No cached values are being shown.':error.message;}
  });
})();
