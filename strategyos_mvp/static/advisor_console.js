...
(function () {
  ...
  function render() {
    ...
    show('advisor-approve-form', record.permissions.can_approve); show('advisor-publish', record.permissions.can_publish); show('advisor-use-template', Boolean(record.approval));
  }
  ...
  $('advisor-use-template').addEventListener('click', function () { action(async function () { var template = await request('/advisor/configurations/' + encodeURIComponent(record.config_id) + '/versions/' + record.version + '/board-template'); $('pack-template').value = JSON.stringify(template, null, 2); $('pack-template').dispatchEvent(new Event('input')); document.getElementById('board-pack').scrollIntoView({ behavior: 'smooth' }); }); });
