...
def _latest_run_public_payload(
    ...
    *,
    ...
    return _sanitize_anonymous_public_payload({
        ...
        "calendar_agenda": dict((summary or {}).get("calendar_agenda") or {}),
        "strategy_enrichment": dict((summary or {}).get("strategy_enrichment") or {}),
...
def _sanitize_anonymous_public_payload(value: Any) -> Any:
    ...
    if isinstance(value, str):
        ...
        if "invoice " in lowered or "inv-" in lowered:
            return "Verified business signal"
...
def _summary_with_reconciled_metrics(
    ...
    *,
    ...
    payload["data_source_status"] = "current_run"
    payload["run_source"] = "current_run"
    if principal_has_any_role(principal_role, "executive"):
        return sanitize_executive_payload(payload)
    return payload
...
def _latest_run_findings_payload(
    ...
    *,
    ...
    payload = {
        ...
        "trend": _trend_card_payload(summary, rows, audit_summary),
        "plan_health": _bounded_plan_health_payload(summary, rows, audit_summary),
        "calendar_agenda": dict(summary.get("calendar_agenda") or {}),
        "strategy_enrichment": dict(summary.get("strategy_enrichment") or {}),
        "publication": publication,
        "strategy_substrate": strategy_substrate,
        "drilldown": _drilldown_contract_payload(
            summary,
...
def _anonymous_public_findings_payload(
    ...
    *,
    ...
    return _sanitize_anonymous_public_payload({
        ...
        "calendar_agenda": dict(safe_summary.get("calendar_agenda") or {}),
        "strategy_enrichment": dict(safe_summary.get("strategy_enrichment") or {}),
        "publication": publication,
        "strategy_substrate": strategy_substrate,
        "drilldown": drilldown,
        "board_portal": board_portal,
...
def _assistant_response_payload(
    *,
    ...
    payload["executive_blocks"] = _executive_answer_blocks(payload)
    return sanitize_executive_payload(payload)
...
def _sanitize_assistant_visible_text(value: Any) -> str:
    ...
    return executive_display_text(text) if executive_text_has_internal_leak(text) else str(text or "").strip()


def _executive_answer_blocks(payload: Mapping[str, Any]) -> dict[str, Any]:
    """Return a renderer-neutral executive answer contract.

    The model supplies reasoning; the server deterministically separates the
    verdict, material figures, recommendation and evidence. No extra fact is
    ...
    sentences = [
        item.strip()
        for item in re.split(r"(?<=[.!?])\s+(?=[A-Z0-9])", answer)
        if item.strip()
    ]
    verdict = sentences[0] if sentences else answer
    recommendation_index = next(
        (
            index
            for index, sentence in enumerate(sentences[1:], start=1)
            if re.search(
                r"\b(?:recommend|should|next|decision|confirm|approve|hold|ask|protect)\b",
                sentence,
                re.IGNORECASE,
            )
        ),
    ...
    recommendation = (
        sentences[recommendation_index] if recommendation_index is not None else None
    )
    detail = " ".join(
        sentence
        for index, sentence in enumerate(sentences)
        if index != 0 and index != recommendation_index
    ) or None
    figures: list[str] = []
    for match in re.findall(
        r"(?:\bSAR\s*~?\s*[\d,.]+\s*[KMB]?\b|[+\-−]?\d+(?:\.\d+)?\s*%|[+\-−]\d+(?:\.\d+)?\s*(?:pts?|days?|weeks?))",
        answer,
        flags=re.IGNORECASE,
    ):
        normalized = re.sub(r"\s+", " ", match).strip()
        if normalized.casefold() not in {item.casefold() for item in figures}:
    ...
    return {
        ...
        "chart": payload.get("chart") if isinstance(payload.get("chart"), Mapping) else None,
    }
...
def data_qa(
    ...
    def _compose_response(
        *,
        ...
        return sanitize_executive_payload(payload) if persona in {"ceo", "executive"} else payload
