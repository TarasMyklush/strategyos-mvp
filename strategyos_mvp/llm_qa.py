...
def plan_claim_retrieval(question: str, *, catalog: list[dict[str, Any]], config: Any) -> dict[str, Any]:
    ...
    raw = _call_openai_compatible_chat(config=config, messages=[
        {"role": "system", "content":
         ...
         'to show existing actual and plan amounts together without calculating a variance; '
         '"calculation" for deterministic ranking, aggregation, variance, percentage difference, '
         'or another request that explicitly applies business arithmetic to supplied facts; '
         ...
         'Showing actual and budget values side by side is a fact lookup. Calculating how far actual is '
...
def answer_question(
    ...
    *,
    ...
    # Neither legacy summaries nor display-safe public packets imply consent.
    from .model_policy import evidence_model_access
    if not evidence_model_access(summary):
        return {
                "matched": False,
