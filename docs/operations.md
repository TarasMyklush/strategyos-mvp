...
| `POST /plans/{plan_id}/versions/{version}/decompose-objective` | Operator, tenant operator or tenant admin | Allocates one complete metric across at least two configured dimensions with exact reconciliation and accountable cells |
...
| `GET /analyses/{analysis_id}/explain?cell_id=...` | Allowed whole-company reader | Deterministic cell answer with ratified plan digest/version and plan/actual evidence references |
