...
| Dimensional intent | `dimensional_plan.py`, `dimensional_intent_api.py`, `dimensional_intent_store.py`, `dimensional_intent_sources.py`, `source_plan_package.py` | Operator evaluator plus authenticated, whole-company API with immutable PostgreSQL revisions, explicit plan-scoped ratifier grants, separate proposer/ratifier identities, registered source eligibility and saved analyses. A typed source-package compiler stages source-defined structure, compiles granular plan/actual contracts, records rounding and ownership rules, and supports month-aligned partial-period comparison against a full-year plan. The reused `/plan` Intent Vault provides review, ratification, imports and saved drift. Active-run selection and graph projection integration remain outstanding. |
